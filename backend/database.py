...
def init_db():
    ...
    _ensure_column(conn, "requests", "work_item_id", "INTEGER REFERENCES work_items(id)")
    ...
    _ensure_column(conn, "expenses", "work_item_id", "INTEGER REFERENCES work_items(id)")
    ...
    _ensure_column(conn, "work_items", "category", "TEXT")
    _ensure_column(conn, "work_items", "status", "TEXT NOT NULL DEFAULT 'idea'")
    _ensure_column(conn, "work_items", "priority", "TEXT NOT NULL DEFAULT 'medium'")
    _ensure_column(conn, "work_items", "target_date", "TEXT")
    _ensure_column(conn, "work_items", "estimated_cost", "REAL")
    _ensure_column(conn, "work_items", "notes", "TEXT")
    _ensure_column(conn, "work_items", "deleted_at", "TEXT")
    _ensure_column(conn, "work_items", "created_at", "TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP")
    _ensure_column(conn, "work_items", "updated_at", "TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP")
    conn.execute(
    ...
    conn.execute("CREATE INDEX IF NOT EXISTS idx_requests_work_item_id ON requests(work_item_id)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_expenses_work_item_id ON expenses(work_item_id)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_work_items_deleted ON work_items(deleted_at)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_work_items_target_date ON work_items(target_date)")
